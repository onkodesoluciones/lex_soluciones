...
import RemitoPDF from '../PDFs/RemitoPDF'
...
const RemitoForm = ({ remito, onClose, onSuccess }) => {
  ...
  const generateRemitoNumber = async () => {
    try {
      const number = await documentsService.generateRemitoNumber()
      setFormData(prev => ({ ...prev, remito_number: number }))
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let savedRemito
      if (remito) {
        savedRemito = await documentsService.updateRemito(remito.id, submitData)
      } else {
        savedRemito = await documentsService.createRemito(submitData)
      }
      ...
      try {
        ...
        const pdfUrl = await generateAndSavePDF(
          ...
          />,
          `remito-${savedRemito.remito_number}-${Date.now()}.pdf`
        )
        
        // Actualizar el remito con la URL del PDF
        await documentsService.updateRemito(savedRemito.id, { pdf_url: pdfUrl })
      } catch (pdfError) {
      ...
      setError('Error al guardar remito: ' + err.message)
      console.error(err)
    } finally {
      setLoading(false)
    }
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? 'Guardando...' : remito ? 'Actualizar' : 'Guardar Remito'}
            </button>
          </div>
        </form>
      </div>
